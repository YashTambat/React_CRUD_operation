...
const handleDelete = (id)=>{
  if(id>0)
  {
    if(window.confirm("Are you sure to delete this item ?",{firstName}))
    {
      const dt=data.filter(item=>item.id!==id);
      setData(dt)
    }
...
const handleClear = ()=>{
  ...
  return (
    <div className="App">
      ...
      <div style={{display:"flex",justifyContent:'center', marginTop:"40px",marginBottom:"20px"}} className='form'>
        <div>
          <label>FirstName: 
        ...
        <div className='butt'>
          {
            !isUpdate?
            <button className='btn btn-primary' id='savebtn' onClick={(e)=>handleSave(e)}>Save</button>
            :
